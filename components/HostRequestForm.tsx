...
export default function HostRequestForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    await createHostRequest(em, picks);
    setBusy(false);
    setDone(true);
    setError('');
  }
    ...
    return (
      <div className="glass" style={{ marginTop: 30 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 10, padding: '6px 2px' }}>
          ...
          <p className="small">
            <B zh="我们会尽快联系你，一起把它办起来。" en="We'll be in touch shortly to plan it together." />
          </p>
          <button
            type="button"
  ...
  return (
    <form className="glass" style={{ marginTop: 30 }} onSubmit={onSubmit}>
      ...
      <p className="fine" style={{ marginTop: 12, fontSize: 12.5, color: 'var(--ink-5)' }}>
        <B
          zh="可多选——我们会带着场地、形式与档期回复你。"
          en="Multiple picks welcome — we'll come back with venues, formats and dates."
        />
      </p>
    </form>
  );
